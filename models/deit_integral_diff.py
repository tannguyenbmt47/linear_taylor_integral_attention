...
class TransformerBlock(nn.Module):
    def __init__(self, embed_dim, num_heads, num_signals=8, mlp_ratio=4.0,
        ...
        self.attn = IntegralDiffAttention(
            embed_dim=embed_dim,
            num_heads=num_heads,
...
class DeiTIntegralDiff(nn.Module):
    """
    DeiT with Integral-Differential Attention.

    Combines differential denoising (subtract signal pairs) and
    integral denoising (average across pairs):
        Attn = softmax( (2/S) * Σ (Q_{2i}K_{2i}^T - λ_i · Q_{2i+1}K_{2i+1}^T) )
    """
...
def deit_tiny_integral_diff(num_classes=1000, **kwargs):
    return DeiTIntegralDiff(
        embed_dim=192, depth=12, num_heads=3, num_signals=8,
        num_classes=num_classes, **kwargs,
    )

def deit_small_integral_diff(num_classes=1000, **kwargs):
    return DeiTIntegralDiff(
        embed_dim=384, depth=12, num_heads=6, num_signals=8,
        num_classes=num_classes, **kwargs,
    )

def deit_base_integral_diff(num_classes=1000, **kwargs):
    return DeiTIntegralDiff(
